...
- A momentum-based LSTM achieved **test AUC 0.45** on unseen data (worse than random).
- **Reverse AUC was 0.55**, indicating signal *inversion* — not signal absence.
- Root cause: a **market regime transition** (momentum → mean-reversion) at the train/test boundary flipped the learned relationship's direction.
- Post-hoc inversion (`1 - prob`) was **rejected as look-ahead bias** — the flip is undetectable at training time.
- Re-designed LSTM inputs with **regime-neutral features** (volatility, volume, candlestick structure), removing momentum-based ones.
- Result: **forward AUC positive across all 4 walk-forward folds** (mean 0.52, std 0.019); the ensemble beats the market baseline in every fold (mean **1.25×**, std 0.076).
- Takeaway: for daily-bar stock prediction, **volatility/volume structure outperforms trend/momentum** as an LSTM signal — momentum inverts under regime shifts, volatility/volume does not.

> 가장 핵심적인 기여는 모델 구조가 아니라, **레짐 전환에 의한 AUC 반전을 발견하고 데이터 누수 없이 분석·해결한 과정**이다. 상세는 아래 "LSTM 피처 설계와 레짐 반전" 참조.
...
| 담당 | 영역 |
|---|---|
| **본인** | **ML 파이프라인 전체** — 데이터 수집 · 피처 설계 · LightGBM/Dual-LSTM 모델 설계 및 학습 · 데이터 누수 방지 검증 · 추론 파이프라인 |
| 팀원 | RAG 기반 Q&A · Frontend |

> 이 README가 설명하는 **수집 → 피처 → 모델 → 검증 → 추론** 전 과정이 본인 담당 영역이다.

---

## 이 프로젝트에서 가장 신경 쓴 것 — 평가의 정직성

금융 시계열은 **데이터 누수(look-ahead bias)** 한 줄로 성능 숫자가 무너진다.
"테스트 구간 정보가 학습에 새어 들어가지 않는다"를 세 가지로 보장했다:

1. **시간순 분할** — 과거(train) → 미래(test), 랜덤 셔플 금지
2. **스케일러 train 구간만 fit** — 미래 분포가 학습에 새는 것 차단 (LSTM)
3. **Walk-Forward 검증** — 여러 시기로 반복해 특정 구간 과적합 배제

→ 상세는 아래 [평가 방법론](#평가-방법론) 참조. (성능 숫자보다 "어떻게 정직하게 측정했나"가 이 프로젝트의 핵심이다.)
...
| **AND 게이트** | 두 모델 모두 통과한 종목만 선별 | `prob_lgb >= 0.54 AND prob_lstm >= 0.49` |
...
### Dual LSTM — 횡단면 상위 분위 (상대 강도)

```
Label = 1  →  5영업일 후 수익률이 "그날 전체 종목 중 상위 30%"
Label = 0  →  그 외
```

- 종목별 N일 후 수익률(`shift(-N)`)을 구한 뒤, **날짜별로 순위(percentile)** 를 매겨 상위 30%만 1 (`_apply_lstm_labels`, `base_processor.py`).
- 절대 수익이 아니라 **같은 날 종목 간 상대 비교** → 시장 전체가 오르거나 내려도 양성 비율이 ~30%로 유지된다(레짐 편향 완화).
- 호라이즌은 20일 → 5일로 단축. 20일은 레짐 전환 구간에서 신호가 더 불안정했다(아래 "LSTM 피처 설계와 레짐 반전" 참조).
...
단일 분할(2025-07~) 결과는 특정 구간 운에 좌우될 수 있어, **Walk-Forward 4폴드**로 검증한 결과를 정식 성능으로 본다.

학습 데이터는 41개 나스닥 종목의 4년치(2022-06 ~ 2026-06) 일봉을 사용한다.

### 앙상블 Walk-Forward (폴드마다 GBM·LSTM 독립 재학습)

| 테스트 구간 | Top3 타율 | 진입 | 베이스라인 | 배수 |
|---|---|---|---|---|
| 2024-07 ~ 2025-01 | 0.4691 | 81 | 0.4195 | 1.12배 |
| 2025-01 ~ 2025-07 | 0.5876 | 354 | 0.4554 | 1.29배 |
| 2025-07 ~ 2026-01 | 0.5018 | 273 | 0.3845 | 1.31배 |
| 2026-01 ~ 2026-07 | 0.5623 | 329 | 0.4376 | 1.29배 |
| **평균** | **0.5302** | — | — | **1.25배 (편차 0.076)** |

- 4개 폴드 전부 베이스라인 초과(1.12~1.31배). 시장 국면이 바뀌어도 한 번도 시장 평균 아래로 내려가지 않는다.
- 스크립트: `app/models/ensemble_wf.py` (AND 게이트 LGBM≥0.54, LSTM≥0.49).

### 모델별 Walk-Forward AUC

| 모델 | AUC | 편차 | 폴드별 |
|---|---|---|---|
| LightGBM | 0.60 | 0.029 | 0.570 / 0.602 / 0.641 / 0.605 |
| Dual LSTM (비모멘텀 피처) | 0.52 | 0.019 | 0.508 / 0.501 / 0.530 / 0.548 |

- LSTM은 4폴드 전부 정방향(>0.5) 유지. 화려하진 않지만 레짐 전환에 흔들리지 않는다.

---

## LSTM 피처 설계와 레짐 반전 (핵심 발견)

LSTM 설계에서 가장 큰 난관은 **레짐 반전(regime inversion)** 이었다.

### 문제

모멘텀 계열 피처(`momentum_3/20/60`, `return_20` 등)로 학습한 LSTM은
2025년 7월 이후 테스트 구간에서 **test AUC 0.45** — 동전 던지기(0.5)보다 나빴다.

```
정방향 AUC 0.449  +  역방향 AUC 0.551  =  1.0
```

역방향이 0.55라는 것은 **신호는 있으나 방향이 정확히 뒤집혔다**는 뜻이다.
원인은 시장 레짐 전환: 학습 구간(모멘텀 우세, "오른 게 더 오른다")과
테스트 구간(평균회귀 우세, "빠진 게 오른다")의 성격이 반대였고,
전환이 하필 train/test 분할선(2025-07)에서 일어나 **학습 시점에 예측 불가**했다.

> ⚠️ `1 - prob`로 뒤집는 것은 test 결과를 보고 방향을 정하는 **look-ahead bias**이므로 금지.

### 시도한 해결책

| 접근 | Walk-Forward 결과 |
|---|---|
| 모멘텀 피처 + 호라이즌 단축(20일→5일) | 여전히 반전 (호라이즌 무관) |
| 모멘텀 + 레짐 인지 피처(`mom_factor`, 5일 평활) | 3폴드 정방향, 1폴드 반전 |
| 모멘텀 + 레짐 인지 피처(`mom_factor`, 3일 평활) | 2폴드 정방향, 2폴드 반전 |
| **모멘텀/추세 피처 전면 제거 (비모멘텀)** | **4폴드 전부 정방향** ✅ |

레짐 인지 피처(`mom_factor` = 모멘텀 전략의 당일 손익으로 추세장/반전장을 측정)를
추가해도 급격한 전환 구간(2025 하반기)은 완전히 방어되지 않았다.

### 최종 결론

추세/모멘텀 신호는 레짐 전환에 본질적으로 취약하다고 판단,
LSTM 입력을 **레짐 중립 피처(변동성·거래량·캔들 구조)** 로 재설계했다.

```
변동성:  volatility_regime_20, volatility_regime_60
거래량:  volume_change, volume_zscore_20, volume_zscore_60
캔들:    candle_body, high_low_spread
```

→ Walk-Forward 4폴드 전부 정방향(평균 AUC 0.52, 편차 0.019),
GBM(기술지표)과 독립적인 신호를 확보해 앙상블에서 실질적으로 기여한다.

### 핵심 결론

> **일봉 주가예측 LSTM에서는 추세·모멘텀보다 변동성·수급 구조의 시계열 패턴이 더 강건하다.**
>
> - 모멘텀 = "방향" 정보 → 레짐(추세장↔반전장)에 따라 의미가 뒤집힘 → AUC 반전
> - 변동성·수급 = "강도·압력" 정보 → 방향과 무관, 어느 레짐에서도 의미가 일정 → 레짐 중립
>
> 당초 LSTM에 추세 탐지를 맡기려 했으나, 데이터는 "이 시장에서 모멘텀 기반 신호는
> 레짐 의존성이 크다"는 것을 일관되게 보였다. 이는 실패가 아니라
> **딥러닝 시계열 신호의 레짐 의존성에 대한 실증적 발견**이다.
...
- 미래 정보는 **타깃(label)에만** 사용한다: GBM은 `shift(-1~-3)` 종가로 "3영업일 내 +2.5%", LSTM은 `shift(-5)` 수익률의 당일 횡단면 상위 30%. 예측 대상이므로 정상이다.
